...
public sealed class ArchiveInfoViewModel : ToolViewModel
{
    ...
    private void GetFlags()
    {
        ...
        using (OleDbConnection oleDbConnection = new(connectionString))
        {
            ...
            while (oleDbDataReader != null && oleDbDataReader.Read())
            {
                ...
                Item item = new(string.Format("$FLAG[{0}]", text[8..]),
    ...
    private void GetTimers()
    {
        ...
        using (OleDbConnection oleDbConnection = new(connectionString))
        {
            ...
            {
                string text = oleDbDataReader.GetValue(0).ToString();
                Item item = new(string.Format("$TIMER[{0}]", text[9..]),
                    oleDbDataReader.GetValue(1).ToString());
                _timer.Add(item);
            }
        }
    ...
    private void GetSignals()
    {
        if (File.Exists(DatabaseFile))
        {
            ...
            {
                string text = oleDbDataReader.GetValue(0).ToString();
                string text2 = text[..text.IndexOf("_", StringComparison.Ordinal)];
                if (text2 != null)
                {
                    if (!(text2 == "IN"))
                    {
                        if (!(text2 == "OUT"))
                        {
                            ...
                            {
                                Item item = new(string.Format("$ANIN[{0}]", text[5..]),
                                    oleDbDataReader.GetValue(1).ToString());
                                _anin.Add(item);
                                LanguageText = LanguageText + item + "\r\n";
                            }
                        ...
                        {
                            Item item = new(string.Format("$OUT[{0}]", text[4..]),
                                oleDbDataReader.GetValue(1).ToString());
                            _outputs.Add(item);
                            LanguageText = LanguageText + item + "\r\n";
                        }
                    ...
                    {
                        Item item = new(string.Format("$IN[{0}]", text[3..]),
                            oleDbDataReader.GetValue(1).ToString());
                        _inputs.Add(item);
                        LanguageText = LanguageText + item + "\r\n";
                    }
